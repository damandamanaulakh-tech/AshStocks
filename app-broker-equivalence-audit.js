(() => {
  ...
  function auditItems() {
    ...
    const sseReady = Boolean(document.querySelector("#upstoxStreamStatus"));
    ...
    return [
      ...
      item("SSE quote stream", sseReady && quoteReady ? "ACTIVE" : sseReady ? "PARTIAL" : "DATA_NEEDED", sseReady ? "Browser has Upstox SSE quote stream client mounted" : "#upstoxStreamStatus not mounted", quoteReady ? "none" : "Need /api/upstox/quote-stream quote event"),
      ...
      item("Broker websocket ticks", "PARTIAL", "SSE quote stream exists; true Upstox websocket SDK/socket is still not implemented", "Optional future: direct Upstox websocket streaming if token scope supports it"),
