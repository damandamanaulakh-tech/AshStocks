(() => {
  ...
  function auditItems() {
    ...
    const paramsReady = Boolean(document.querySelector("#uwParameterKeyBoard") || document.querySelector("#uwPianoQuick") || document.querySelector("#parameterPiano"));
    ...
    const dataNeededRows = r.filter((row) => row.decision === "DATA_NEEDED" || row.fetch_error);
    ...
    return [
      ...
      item("Candle parameters 681-800", candleRows.length ? "ACTIVE" : document.querySelector("#uwCandleTriggerTape") ? "PARTIAL" : "DATA_NEEDED", candleRows.length ? `${candleRows.length} rows have candle evidence` : "Candle tape exists but row candle evidence is not present", candleRows.length ? "none" : "Upstox historical candles or server candle_status rows"),
      item("Upstox quote and depth", quoteReady ? "ACTIVE" : document.querySelector("#uwMarketWatchPulse") ? "PARTIAL" : "DATA_NEEDED", quoteReady ? `${state.quoteSeen} quote events, last ${state.lastQuoteAt || "unknown"}` : "Quote module loaded without quote evidence yet", quoteReady ? "none" : "Valid Upstox token and quote request during market/data availability"),
      item("Broker market watch", document.querySelector("#brokerMarketWatchPulse") || document.querySelector("#uwMarketWatchPulse") ? "ACTIVE" : "DATA_NEEDED", document.querySelector("#brokerMarketWatchPulse") ? "Broker market watch pulse mounted" : document.querySelector("#uwMarketWatchPulse") ? "Dashboard quote pulse mounted" : "No market watch pulse in DOM", "Mount watchlist quote strip and table"),
      item("Symbol workspace and chart", document.querySelector("#uwSymbolWorkspace") ? "ACTIVE" : "PARTIAL", document.querySelector("#uwSymbolWorkspace") ? "Selected symbol workspace mounted" : "Main dashboard has selected-stock mini chart only", document.querySelector("#uwSymbolWorkspace") ? "none" : "Full broker symbol chart panel"),
      item("Buy Sell GTT ticket", document.querySelector("#uwOrderTicket button[data-paper-action]") || document.querySelector("#brokerHubOrderBox") ? "ACTIVE" : "PARTIAL", document.querySelector("#uwOrderTicket") ? "Paper ticket mounted and lifecycle script can bind actions" : "Ticket not visible on current view", "Keep only paper order POST, no broker write"),
      item("Order book and trade book", orders.length || trades.length ? "ACTIVE" : document.querySelector("#riskOrdersPanel") || document.querySelector("#uwPaperLedgerPanel") ? "PARTIAL" : "DATA_NEEDED", `${orders.length} orders, ${trades.length} trades in ledger`, orders.length || trades.length ? "none" : "Create paper order to prove lifecycle"),
      item("Positions and exits", positions.length ? "ACTIVE" : document.querySelector("#riskPositionsPanel") ? "PARTIAL" : "DATA_NEEDED", `${positions.length} positions in paper ledger`, positions.length ? "none" : "Paper filled BUY position or imported ledger position"),
      item("GTT target stop plans", gtt.length ? "ACTIVE" : document.querySelector("#riskGttPanel") ? "PARTIAL" : "DATA_NEEDED", `${gtt.length} GTT plans in paper ledger`, gtt.length ? "none" : "Create paper GTT from signal/ticket"),
