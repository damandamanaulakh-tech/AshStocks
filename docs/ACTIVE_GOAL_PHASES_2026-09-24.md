...
| --- | --- | --- |
| 1. Baseline, asset review and connection map | Verify repository/live commit; inventory all release assets; inspect actual formats/fields/dates/symbols/quality; map each usable dataset to the intended consumer; identify backup/access gaps. | Small batch 1A records integrity and selected CSV evidence. Full content/identity/corporate-action validation, screenshot mapping and live Mongo baseline remain open. |
...
Finish the Phase 1 asset-to-consumer audit and reproducible validation/rejected-row catalog. In parallel obtain only the scoped authenticated Mongo alarm/metrics and dashboard evidence when accessible. Before any production cleanup show exact targets and request confirmation; before any deployment resolve the service/release target. Keep goal active until the live acceptance gates are genuinely complete.
