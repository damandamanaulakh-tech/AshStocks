...
| --- | --- | --- |
| 1. Baseline, asset review and connection map | Verify repository/live commit; inventory all release assets; inspect actual formats/fields/dates/symbols/quality; map each usable dataset to the intended consumer; identify backup/access gaps. | Batch 1A published at `7996da6`; batch 1B adds a reproducible bounded OHLCV dry run and 20 tests (see checkpoint below). Full content/identity/corporate-action validation, screenshot mapping and live Mongo baseline remain open. |
...
Read `OHLCV_VALIDATION_CHECKPOINT_2026-09-24.md` for the completed single-family validator, actual rejected-row report and remaining scope. Continue Phase 1C with other data-family schemas, historical identity/adjustment checks and a capacity-reviewed storage/consumer contract. Inspect existing Mongo history before proposing duplicate ingestion: absence of a Node adapter does not establish absence of stored data. In parallel obtain the scoped authenticated Mongo alarm/metrics and dashboard evidence when accessible. Before any production cleanup show exact targets and request confirmation; before deployment resolve the service/release target. Keep goal active until the live acceptance gates are genuinely complete.
