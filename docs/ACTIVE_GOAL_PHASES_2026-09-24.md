...
Read the OHLCV, identity/workbook, root-CSV, additional-asset, institutional-correction, research/report and formula-parity checkpoints for completed diagnostics, approved code and explicit limitations. Root binary XLS/HTML/metadata and seven research archives are now read; PDF numerical content remains partial. Next bounded content work is the 42 small participant CSVs and context-checked duplicate reuse, followed by schema/month/year batches of the large monthly FPI history and remaining AM07/aqu/bhavcopy/derived sources. The approved formula-ID/warmup/null/availability batch is complete offline; discuss unresolved formula contracts, additional strategy activation or historical lift/risk-constant changes before any such edit. Do not promote archived backtests to live proof. Neither offline correction is a current feed or Mongo success claim. Continue safe content review while obtaining scoped authenticated Mongo alarm/history metrics and the missing dashboard screenshot. Before new ingestion inspect existing Mongo history and capacity; absence of a Node adapter does not establish absence of stored data. Before production cleanup show exact targets and request confirmation; before deployment resolve the service/release target. Keep the goal active until the live acceptance gates are genuinely complete.
