...
const PAPER_TRADER_VERSION = "ashstocks-paper-trader-v0.1";

const PAPER_TRADER_PARAMETERS = Object.freeze([
  { key: "scanner_score", group: "Core", label: "Scanner score", purpose: "base candidate strength", weight: 35 },
  { key: "momentum_6m_12m", group: "Core", label: "6M/12M momentum", purpose: "trend confirmation", weight: 18 },
  { key: "target_left", group: "Rotation", label: "Target room left", purpose: "avoid buying exhausted stocks", weight: 12 },
  { key: "liquidity", group: "Execution", label: "ADV/turnover", purpose: "paper order feasibility", weight: 10 },
  { key: "theme_heat", group: "Narrative", label: "Theme heat", purpose: "AI/defence/rail/infra/energy/IT/PSU basket tagging", weight: 10 },
  { key: "event_resilience", group: "Event", label: "Event resilience", purpose: "prefer stocks holding trend through volatile regimes", weight: 8 },
  { key: "risk_penalty", group: "Risk", label: "Volatility and data penalty", purpose: "cut noisy or incomplete names", weight: -7 },
  { key: "paper_rotation", group: "Portfolio", label: "Sell/replace discipline", purpose: "exit target hit, stop hit, stale underperformer", weight: 0 }
]);

const THEME_KEYWORDS = Object.freeze([
  { theme: "AI / Digital", words: ["TECH", "INFOTECH", "SOFTWARE", "DIGITAL", "DATA", "TATA CONSULTANCY", "TCS", "INFOSYS", "HCL", "WIPRO", "PERSISTENT", "LTIMINDTREE", "MPHASIS"] },
  { theme: "Defence / Aerospace", words: ["DEFENCE", "AEROSPACE", "BHARAT ELECTRONICS", "BEL", "HAL", "COCHIN", "SHIP", "BEML"] },
  { theme: "Rail / Infra", words: ["RAIL", "IRFC", "IRCTC", "RVNL", "RITES", "INFRA", "LARSEN", "L&T", "CONSTRUCTION", "ENGINEERS"] },
  { theme: "Green Energy / Power", words: ["GREEN", "RENEW", "SOLAR", "POWER", "ENERGY", "NTPC", "ADANI GREEN", "TATA POWER", "SUZLON"] },
  { theme: "EV / Auto", words: ["AUTO", "MOTOR", "ELECTRIC", "BATTERY", "EXIDE", "AMARA", "TATA MOTORS", "M&M", "BAJAJ AUTO", "EICHER"] },
  { theme: "Banks / Financials", words: ["BANK", "FINANCE", "HDFC", "ICICI", "KOTAK", "AXIS", "SBIN", "BAJAJ FINANCE", "NBFC"] },
...
function paperTraderStatusPayload(state = defaultState()) {
  ...
  return {
    ...
    paper_only: true,
    live_orders: false,
    parameters: PAPER_TRADER_PARAMETERS,
    status: paperTrader,
...
function buildPaperTraderPlan(scan, state = defaultState(), options = {}) {
  ...
  const historyItem = {
    ...
    scanned: rows.length,
    buy_queue: buyQueue.length,
    sell_queue: sellQueue.length,
    data_needed: ranked.filter((row) => row.decision === "DATA_NEEDED").length
  };
...
function enrichPaperCandidate(row = {}, settings) {
  ...
  const paperScore = round(
    score * 0.35 + momentum * 0.18 + quality * 0.10 + Math.max(0, Math.min(100, targetLeft * 3)) * 0.12 + liquidity * 0.10 + themeHeat * 0.10 + eventResilience * 0.08 - riskPenalty,
    2
  );
  ...
  return {
    ...row,
    themes,
    theme_heat: round(themeHeat, 2),
    event_resilience: round(eventResilience, 2),
    paper_score: paperScore,
    paper_ready: paperReady,
    watch_ready: watchReady,
    target_price: targetPrice,
    stop_price: stopPrice,
    target_pct: round(targetPct, 2),
    stop_loss_pct: settings.stopLossPct,
    paper_reason: paperReason(row, themes, targetLeft, paperScore)
  };
}

function detectThemes(row = {}) {
  const text = `${row.symbol || ""} ${row.name || ""} ${row.sector || ""}`.toUpperCase();
...
function evaluatePaperPosition(position, rows, settings, asOf) {
  ...
  if (current && target && current >= target * (settings.targetHitPct / 100)) {
    action = "TARGET_HIT";
    reason = "target progress reached rotation threshold";
  }
  if (current && stop && current <= stop) {
    action = "STOP_HIT";
    reason = "stop loss reached";
  }
  if (row.paper_score !== undefined && row.paper_score < settings.replaceBelowScore && action === "HOLD") {
    action = "REPLACE";
    reason = "paper score fell below replace threshold";
  }
  return {
    ...position,
    ...
    latest_paper_score: row.paper_score ?? null,
    action,
    reason,
    checked_at: asOf,
    paper_only: true
...
function buildWatchlists(ranked, investable, watch) {
  ...
  for (const row of ranked) {
    for (const theme of row.themes || []) {
      ...
      byTheme.get(theme).push(miniCandidate(row));
    }
  }
  return {
    morning_top_50: investable.slice(0, 50).map(miniCandidate),
    buy_queue_30: investable.slice(0, 30).map(miniCandidate),
    event_resilient: ranked.filter((row) => row.event_resilience >= 65).slice(0, 30).map(miniCandidate),
    target_room: ranked.filter((row) => finiteOr(row.target_potential?.potential_left_pct, 0) >= 20).slice(0, 30).map(miniCandidate),
    watch_not_buy: watch.slice(0, 40).map(miniCandidate),
    data_needed: ranked.filter((row) => row.decision === "DATA_NEEDED").slice(0, 40).map(miniCandidate),
    themes: Object.fromEntries([...byTheme.entries()].map(([theme, rows]) => [theme, rows.slice(0, 20)]))
  };
}
...
function miniCandidate(row) {
  return {
    ...
    stop_price: row.stop_price,
    themes: row.themes || [],
    reason: row.paper_reason || row.reason || ""
  };
}
`;
...
const PAPER_TRADER_ROUTES = String.raw`
      ...
      if (url.pathname === "/api/paper-trader/run") {
        ...
        let scan;
        if (body.useUpstox !== false && ENV.UPSTOX_ACCESS_TOKEN) {
          scan = await runUpstoxScanner(body, resolved.universe);
        }
        if (!scan || scan.ok === false) {
          scan = runScanner(resolved.universe, { ...(body.settings || {}), source: resolved.source, holdings: body.holdings || state.paperTrader?.positions || [] });
        }
