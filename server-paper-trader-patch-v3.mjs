...
const PAPER_TRADER_VERSION = "ashstocks-paper-trader-v0.3";
const PAPER_TRADER_PARAMETERS = Object.freeze([
  { key: "scanner_score", group: "Core", label: "Scanner score", weight: 30 },
  { key: "momentum", group: "Core", label: "6M/12M momentum", weight: 20 },
  { key: "quality", group: "Core", label: "Quality/liquidity quality", weight: 10 },
  { key: "target_room", group: "Rotation", label: "Target room", weight: 10 },
  { key: "theme_heat", group: "Narrative", label: "India hot-pocket theme", weight: 10 },
  { key: "event_resilience", group: "Event", label: "Event/crash resilience", weight: 10 },
  { key: "liquidity", group: "Execution", label: "Rupee turnover", weight: 10 }
]);
...
function enrichPaperCandidate(row = {}, settings) {
  ...
  const paperReady = Boolean(close && row.decision !== "DATA_NEEDED" && paperScore >= settings.replaceBelowScore);
  const watchReady = Boolean(close && row.decision !== "DATA_NEEDED" && paperScore >= 25);
  const targetPct = Math.max(12, Math.min(80, targetLeft >= 8 ? targetLeft : settings.targetDefaultPct));
...
function paperReason(row, themes, targetLeft, paperScore) {
  const parts = ["paper_score " + paperScore];
  if (row.decision) parts.push(row.decision === "BLOCKED" ? "scanner-review" : row.decision);
  ...
  if (row.reason) parts.push(row.reason.replace(/target-potential hard gate failed;?\s*/i, "target reviewed; "));
  return parts.join("; ");
}
...
function buildWatchlists(ranked, investable, watch) {
  ...
  return { selected_30: investable.slice(0, 30).map(miniCandidate), morning_top_50: investable.slice(0, 50).map(miniCandidate), event_resilient: ranked.filter((row) => row.event_resilience >= 65).slice(0, 30).map(miniCandidate), target_room: ranked.filter((row) => finiteOr(row.target_potential?.potential_left_pct, 0) >= 20).slice(0, 30).map(miniCandidate), watch_not_buy: watch.slice(0, 40).map(miniCandidate), data_needed: ranked.filter((row) => row.decision === "DATA_NEEDED").slice(0, 40).map(miniCandidate), themes: Object.fromEntries([...byTheme.entries()].map(([theme, rows]) => [theme, rows.slice(0, 20)])) };
}
function miniCandidate(row) {
  return { symbol: row.symbol, name: row.name, sector: row.sector || "Unmapped", decision: row.decision, score: row.score, paper_score: row.paper_score, close: row.close, target_price: row.target_price, stop_price: row.stop_price, readiness: row.paper_ready ? (["SELECT", "WATCH"].includes(row.decision) ? "READY" : "REVIEW") : "WATCH", themes: row.themes || [], reason: row.paper_reason || row.reason || "" };
}
`;
const PAPER_TRADER_ROUTES = String.raw`
      if (url.pathname === "/api/paper-trader/parameters") { json(res, 200, { ok: true, engine: PAPER_TRADER_VERSION, paper_only: true, live_orders: false, parameters: PAPER_TRADER_PARAMETERS }); return; }
      if (url.pathname === "/api/paper-trader/status") { const store = await getStore(); const state = await store.getState(); json(res, 200, paperTraderStatusPayload(state)); return; }
