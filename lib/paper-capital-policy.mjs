...
export function validatePaperCapitalRegistry(registry) {
  ...
  const exact = {
    ...
    maxBuysPerRun: 80,
    affordableOpenPositionsAtMinimum: 500,
    initialAffordableOpenPositionsAfterEntryCost: 499,
  };
  ...
  if (registry?.execution?.transactionCostOneWayPct !== 0.08) {
    failures.push("execution.transactionCostOneWayPct must be 0.08");
  }
  if (registry?.exposureGovernor?.drawdownTriggerPct !== -18) {
    failures.push("exposureGovernor.drawdownTriggerPct must be -18");
  }
  const entryDebit = capital.minimumEntryValue * (1 + registry.execution.transactionCostOneWayPct / 100);
  if (Math.floor(capital.initialCapitalInr / entryDebit) !== capital.initialAffordableOpenPositionsAfterEntryCost) {
    failures.push("initialAffordableOpenPositionsAfterEntryCost must include the approved BUY cost");
  }
...
export function paperCapitalPolicyFromRegistry(registry) {
  ...
  return Object.freeze({
    ...
    initialAffordableOpenPositionsAfterEntryCost: capital.initialAffordableOpenPositionsAfterEntryCost,
    cashBufferPct: capital.cashBufferPct,
    maximumPortfolioHeatPct: capital.maxPortfolioHeatPct,
    portfolioDrawdownTriggerPct: registry.exposureGovernor.drawdownTriggerPct,
    transactionCostOneWayPct: registry.execution.transactionCostOneWayPct,
