...
The executable capital source is `config/ash-stock-parameters.v2.2.json#paperCapital`: ₹5 crore starting capital, ₹1 lakh minimum BUY, 80 candidates/fills per cycle, and a 500-position hard ceiling. At the approved 0.08% BUY cost, 499 minimum-size positions are initially affordable; the 500th requires sufficient net gains. Quarter-Kelly remains paper-only and can reduce the base cap after its evidence gates pass.
