...
for (const [file, checks] of Object.entries({
  ...
  "server-upstox-institutional-patch.mjs": ["UPSTOX_INSTITUTIONAL_VERSION", "/api/upstox/institutional-flow", "/v2/fundamentals/:isin/share-holdings", "/v2/market/fii", "/v2/market/dii", "fii_holding_pct", "fii_change_pp", "fii_cash_5d_net_cr", "latest_reported_quarter: true", "No stock-wise daily FII cash-flow attribution", "institutional_analytics_enabled: true", "token_printed: false"],
  ...
  "app-ashstocks-terminal-filter-bridge.js": ["data-terminal-filter", "SELECT", "WATCH", "CANDLE", "DATA_NEEDED", "No rows match this real scanner filter", "/api/scanner/run"],
  ...
  "upstox-stream-client.css": [".upstox-stream-status", ".upstox-stream-status button", "#2563eb"],
  ...
  "app-upstox-reasoning-dock.js": ["#uwReasoningDock", "/api/scanner/run", "/api/paper-trader/orders", "ashstocks:upstox-quote", "quoteStateFor", "quoteStatus", "Quote proof", "Depth", "Reason, Verify, Execute", "Decision Evidence", "Parameter Gates", "Paper Execution", "broker_write_enabled: false"],
...
mustLoad("index.html", "./styles.css?v=20260826.1");
mustLoad("index.html", "./app.js?v=20260826.1");
...
mustMatch("app-ashstocks-terminal-filter-bridge.js", /matches[\s\S]*SELECT[\s\S]*WATCH[\s\S]*DATA_NEEDED[\s\S]*CANDLE/, "terminal watch filters use scanner row state");
